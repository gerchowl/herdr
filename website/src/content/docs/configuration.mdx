...
`pane_header` (default `true`) reserves a header strip at the top of agent panes — the pane's terminal shrinks so nothing is ever covered. Row 1 shows context: `project · worktree ·  branch`, unpushed/unpulled counts (`↑n`/`↓n`), and the branch's GitHub PR state (`#N ⊙` open, `◐` draft, `✓` merged, `✗` closed — polled via `gh` every two minutes, omitted when `gh` or a PR is absent). The remaining rows show the **last submitted prompt**. The reservation latches the first time an agent is seen in a pane and stays stable from then on. Panes too small to give up rows skip the header.
